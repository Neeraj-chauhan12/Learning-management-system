...
const CourseData = () => {
  ...
  const isPurchased = profile?.user?.enrollCourse?.some(
    (course) => course._id === courseId,
  );

  const { data, isLoading: _isLoading } = useGetLectureQuery(courseId);

  const { data: courseData, isLoading: _isCourseLoading } =
      ...
      <div className="bg-slate-950 text-white pt-28 pb-12 px-4">
        <div className="mx-auto max-w-7xl">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            Advanced Web Development Mastery
          </h1>
          <p className="text-lg text-white/90 mb-6 max-w-2xl">
            Learn professional web development from industry experts with
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-semibold">
                  {courseData?.course?.enrolledStudents?.length || 0}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-semibold">
                  {courseData?.course?.updatedAt?.slice(0, 10) || "N/A"}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600">
                    Expert Instructor • 3+ Years Experience
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3">
                {data?.lectures?.map((item, idx) => (
                  ...
                  >
                    ...
                    >
                      <p
                        className={`font-medium ${item?.locked ? "text-slate-500" : "text-slate-950"}`}
                      >
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="border-t border-slate-200 pt-6">
                  {profile?.user?.role === "instructor" ? (
                    <button
                      onClick={() => navigation(`/edit/${courseId}`)}
                      className="w-full rounded-xl bg-gray-600 py-3 font-semibold text-white shadow-lg shadow-gray-500/20"
                    >
                      Edit Course
                    </button>
                  ) : isPurchased ? (
                    <button
                      onClick={() => navigation(`/my-learning`)}
